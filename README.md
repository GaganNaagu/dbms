...
10. ~~A trigger that prevents driver with total damage amount >rs.50,000 from owning a car.~~
...
8. ~~A trigger that prevents warehouse details from being deleted if any item has to be shipped from that warehouse.~~
9. ~~A view that shows the warehouse name from where the kumar’s order is been shipped.~~
...
8. ~~Create a view to display course related books from course_adoption and text book table using book_ISBN.~~
9. ~~Create a trigger such that it Deletes all records from enroll table when course is deleted .~~
